...
if x == 1:
    ...
    while running:
        ...
        if keys[pygame.K_LEFT] or keys[pygame.K.A]:
            bot_angle += bot_rot_speed
        if keys[pygame.K_RIGHT] or keys[pygame.K.D]:
            bot_angle -= bot_rot_speed

        delta_x = speed * math.cos(math.radians(-bot_angle))
        delta_y = speed * math.sin(math.radians(-bot_angle))

        if keys[pygame.K_UP] or keys[pygame.K.W]:
            bot_x += delta_x
            bot_y += delta_y
        if keys[pygame.K_DOWN] or keys[pygame.K.S]:
            bot_x -= delta_x
            bot_y -= delta_y
